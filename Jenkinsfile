    ...
    stage('Docker Push') {
      ...
      steps {
        ...
        stage("ZAProxy") {
          ...
          sh 'url=http://zippyops:zippyops@$ip:8080/newapp-0.0.1-SNAPSHOT/'
				  sh '''
				  sed -i \'s/http\\:\\/\\/15\\.206\\.11\\.209/$url/g\' /var/jenkins_home/zaproxy-job.yaml 
          kubectl apply -f /var/jenkins_home/zaproxy-job.yaml
				  '''             
            }
           }
          } 
